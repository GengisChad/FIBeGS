...
BEGIN
  ...
  LOOP
    stats := jsonb_set(stats, ARRAY[rec.uid], '{"w":0,"l":0,"d":0,"gw":0,"gl":0}'::jsonb);
  ...
  LOOP
    IF m.phase = 'swiss' THEN
      ...
      END IF;
    ELSE
      -- Elimination matches (top_cut, tiebreaker, pre_top_cut)
      IF m.p1 IS NULL OR m.p2 IS NULL THEN CONTINUE; END IF;
      IF NOT stats ? m.p1 THEN stats := jsonb_set(stats, ARRAY[m.p1], '{"w":0,"l":0,"d":0,"gw":0,"gl":0}'::jsonb); END IF;
      IF NOT stats ? m.p2 THEN stats := jsonb_set(stats, ARRAY[m.p2], '{"w":0,"l":0,"d":0,"gw":0,"gl":0}'::jsonb); END IF;
