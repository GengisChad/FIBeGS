...
BEGIN
  -- Trova il destinatario (l'altro utente della chat)
  ...
  VALUES (
    v_recipient,
    'private_message',
    COALESCE(v_sender_name, 'Nuovo messaggio'),
  ...
  PERFORM net.http_post(
    ...
    body := jsonb_build_object(
      'user_ids', jsonb_build_array(v_recipient::text),
      'title', COALESCE(v_sender_name, 'Nuovo messaggio'),
      'body', 'Ti ha inviato un messaggio',
      'data', jsonb_build_object(
        'type', 'private_message',
        'chat_id', NEW.chat_id::text,
        'link', '/?chat=' || NEW.chat_id::text,
