...
DROP TRIGGER IF EXISTS trg_00_enforce_tournament_payment_registration_status ON public.tournament_registrations;
CREATE TRIGGER trg_00_enforce_tournament_payment_registration_status
...
AS $$
  ...
  WHERE tr.tournament_id = ANY(_tournament_ids)
    AND (
      ((COALESCE(t.entry_fee, 0) > 0 OR NULLIF(BTRIM(COALESCE(t.payment_method, '')), '') IS NOT NULL) AND tr.status IN ('confirmed', 'pending_payment'))
      OR ((COALESCE(t.entry_fee, 0) <= 0 AND NULLIF(BTRIM(COALESCE(t.payment_method, '')), '') IS NULL) AND tr.status = 'confirmed')
    )
...
WHERE t.id = tr.tournament_id
  ...
  AND (COALESCE(t.entry_fee, 0) > 0 OR NULLIF(BTRIM(COALESCE(t.payment_method, '')), '') IS NOT NULL)
  AND tr.status = 'confirmed'
  AND tr.confirmed_at IS NULL;