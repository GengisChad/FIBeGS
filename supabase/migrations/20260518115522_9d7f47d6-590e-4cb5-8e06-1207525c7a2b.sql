...
BEGIN
  ...
  -- Treat tournament as paid if entry_fee > 0 OR there are any pending_payment entries
  SELECT EXISTS (
    SELECT 1 FROM public.tournament_registrations
    WHERE tournament_id = t_record.id AND status = 'pending_payment'
  ) INTO has_pending_payment;

  IF t_record.entry_fee > 0 OR has_pending_payment OR freed_status = 'pending_payment' THEN
    occupied_statuses := ARRAY['confirmed','pending_payment'];
    -- Promote to the same status as the freed slot to avoid skipping the payment step
    target_status := COALESCE(freed_status, 'pending_payment');
  ELSE
