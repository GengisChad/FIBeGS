...
-- Fix recalculate_all_rankings to only consider tournaments within active season date range
...
DECLARE
  _season_start date;
BEGIN
  -- Get the active season start date
  ...
  -- Recalculate regular profiles (only tournaments within active season)
  UPDATE profiles p
  ...
  -- Recalculate child profiles (only tournaments within active season)
...
-- Fix finalize_tournament_points to only consider tournaments within active season when updating profile totals
...
BEGIN
  ...
  -- UPDATE PROFILE TOTALS - FILTER BY ACTIVE SEASON DATE RANGE
  UPDATE profiles p
  SET points = COALESCE(sub.total, 0),
      wins = COALESCE(sub.win_count, 0),
      updated_at = now()
  ...
  -- Also reset profiles that HAD points but now have none in this season
  ...
  -- Reset child profiles that had points but now have none
