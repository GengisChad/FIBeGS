...
-- Patch recalc function: bye game wins = 4 instead of 2 (BYE_POINTS already 4)
...
declare
  ...
  BYE_GAME_WINS constant integer := 4;
  _tc_final_round int;
begin
    ...
    if m.phase = 'swiss' then
      if m.p1 is not null and m.p2 is null then
        if stats ? m.p1 then
          ...
          stats := jsonb_set(stats, array[m.p1, 'gw'], to_jsonb((stats->m.p1->>'gw')::int + BYE_GAME_WINS));
...
-- Fix bye scores only for tournaments currently in progress (not completed)
UPDATE public.tournament_matches tm
SET player1_score = 4
FROM public.tournaments t
WHERE tm.tournament_id = t.id
  AND t.status IN ('swiss','top_cut','pending')
  AND tm.status = 'completed'
  AND tm.player2_id IS NULL
  AND tm.player1_id IS NOT NULL
  AND tm.player1_score = 2;

-- Recalc standings only for in-progress tournaments
DO $$
DECLARE t_id uuid;
BEGIN
  FOR t_id IN
    SELECT id FROM public.tournaments WHERE status IN ('swiss','top_cut','pending')
  LOOP
