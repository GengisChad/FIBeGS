...
export const GlobalChatView = ({ onBack, hideHeader }: Props) => {
  const { user } = useAuth();
  const { isBlocked } = useModerationStatus();
  ...
  const loadProfiles = async (ids: string[]) => {
    ...
    setProfiles(prev => { const n = { ...prev }; (data || []).forEach((p: any) => n[p.user_id] = p); return n; });
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const { data } = await (supabase as any).from("global_messages")
        .select("id, sender_id, content, created_at, edited_at").order("created_at", { ascending: false }).limit(100);
      if (cancelled) return;
  ...
  useEffect(() => {
    const ch = supabase.channel("global-msg")
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "global_messages" },
        (payload: any) => {
          const m = payload.new as Msg;
          setMessages(prev => prev.map(p => p.id === m.id ? { ...p, ...m } : p));
        })
      .on("postgres_changes", { event: "DELETE", schema: "public", table: "global_messages" },
        (payload: any) => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const v = text.trim();
    if (!v || !user) return;
    if (isBlocked) { toast.error("Non puoi scrivere mentre sei in moderazione"); return; }
    setText("");
    const { error } = await (supabase as any).from("global_messages").insert({ sender_id: user.id, content: v });
    if (error) toast.error(error.message);
  };

  const editMsg = async (id: string, next: string) => {
    if (!user) return;
    const { error } = await (supabase as any).from("global_messages")
  ...
  const deleteMsg = async (id: string) => {
    if (!user) return;
    const { error } = await (supabase as any).from("global_messages").delete().eq("id", id).eq("sender_id", user.id);
    if (error) { toast.error(error.message); return; }
    ...
    <div className="flex flex-col h-full">
      {!hideHeader && (
        <div className="flex items-center gap-2 px-3 py-2 border-b bg-card">
          ...
          <div className="flex-1 text-sm font-semibold">Chat Globale Italia</div>
        </div>
      )}
      ...
      <form onSubmit={submit} className="p-2 border-t flex gap-2 bg-card">
        <Input value={text} onChange={e => setText(e.target.value)} placeholder={isBlocked ? "Non puoi scrivere" : "Scrivi al pubblico..."} disabled={isBlocked} />
        <Button type="submit" disabled={isBlocked}><Send size={14} /></Button>
      </form>
    </div>
  );
};
