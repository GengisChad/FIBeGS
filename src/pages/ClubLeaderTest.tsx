...
import { Users as Shield, CheckCircle, Clock, Pencil, Plus, Trash2, Upload, ImageIcon, Video, Settings, ArrowLeft } from "lucide-react";
...
const ClubLeaderTest = () => {
  const { user } = useAuth();
  const { isAdmin } = useAdmin();
  const { passed, cooldownUntil, loading: statusLoading } = useRefereeTestStatus("club_leader");
  ...
  const fetchQuestions = async () => {
    ...
    if (isAdmin) {
      ...
      const { data } = await supabase.rpc("get_test_answers_safe", { _test_type: "club_leader" });
  ...
  const handleSaveQuestion = async () => {
    ...
    if (editingQuestion) {
      ...
      // Insert new
      const { data: newQ } = await (supabase as any).from("referee_test_questions").insert({
        question_text: qForm.question_text,
        media_url: qForm.media_url || null,
        media_type: qForm.media_type,
        is_multiple_choice: qForm.is_multiple_choice,
        sort_order: questions.length,
        test_type: "club_leader",
      }).select().single();
