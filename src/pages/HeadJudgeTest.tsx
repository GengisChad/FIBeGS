...
import { Crown, CheckCircle, Clock, Pencil, Plus, Trash2, Upload, Settings, ArrowLeft, ShieldAlert } from "lucide-react";
...
const TEST_TYPE = "head_judge";

const HeadJudgeTest = () => {
  const { user } = useAuth();
  const { isAdmin } = useAdmin();
  const refereeStatus = useRefereeTestStatus("referee");
  const { passed, cooldownUntil, loading: statusLoading } = useRefereeTestStatus(TEST_TYPE);
  ...
  const isEligible = refereeStatus.passed || isAdmin;
  ...
  const fetchQuestions = async () => {
    ...
    if (isAdmin) {
      ...
      const { data } = await supabase.rpc("get_test_answers_safe", { _test_type: TEST_TYPE });
  ...
  const handleSubmitTest = async () => {
    ...
    if (res.error === "not_eligible") {
      toast({ title: "Non sei idoneo per questo test", description: "Devi aver ottenuto il 100% nel test arbitri.", variant: "destructive" });
      return;
    }
  ...
  const handleMediaUpload = async (file: File): Promise<string | null> => {
    ...
    const path = `questions/hj_${Date.now()}.${ext}`;
  ...
  const handleSaveQuestion = async () => {
    ...
    if (editingQuestion) {
      await supabase.from("referee_test_questions" as any).update({
      ...
      const { data: newQ } = await supabase.from("referee_test_questions" as any).insert({
        question_text: qForm.question_text,
        media_url: qForm.media_url || null,
        media_type: qForm.media_type,
        is_multiple_choice: qForm.is_multiple_choice,
  ...
  const handleDeleteQuestion = async (qId: string) => {
    ...
    await supabase.from("referee_test_questions" as any).delete().eq("id", qId);
    fetchQuestions();
  ...
  // Non-eligible users see access denied
  if (!isEligible && !statusLoading && !refereeStatus.loading) {
    return (
      <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="pt-20 pb-16">
          <div className="container mx-auto px-4 max-w-4xl text-center py-20">
            ...
            <h1 className="text-2xl font-bold mb-2">Accesso non consentito</h1>
            <p className="text-muted-foreground mb-6">
              Per accedere al test Head Judge devi prima superare il test Judge.
            </p>
            <Button asChild>
              <Link to="/rules">Torna al Regolamento</Link>
            </Button>
          </div>
        </div>
        <Footer />
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
                ...
                <div className="border-t border-border pt-4 space-y-4">
                  {passed ? (
                    <div className="flex items-center gap-2 text-amber-500 font-semibold">
                      <Crown size={20} />
                      SEI HEAD JUDGE - Hai già superato il test
                    </div>
                  ) : cooldownUntil ? (
                    ...
                    <>
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <label htmlFor="accept-hj" className="text-sm cursor-pointer">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                    <div className="h-full bg-amber-500 transition-all" style={{ width: `${((idx + (canNext ? 1 : 0)) / total) * 100}%` }} />
                  </div>
                          ...
                          return (
                            <button
                              ...
                              className={`w-full text-left px-4 py-3 rounded-lg border transition-colors ${
                                selected ? "border-amber-500 bg-amber-500/10 text-foreground" : "border-border hover:border-amber-500/50"
                              }`}
                            >
                              {a.answer_text}
                            </button>
                ...
                <div className="flex justify-between items-center pt-2">
                  ...
                  {isLast ? (
                    <Button disabled={answeredCount < total || submitting} onClick={handleSubmitTest} className="bg-amber-600 hover:bg-amber-700">
                      {submitting ? "Invio..." : "Invia test"}
                    </Button>
                  ) : (
                    <Button disabled={!canNext} onClick={() => setCurrentStep(s => s + 1)} className="bg-amber-600 hover:bg-amber-700">Avanti</Button>
                  )}
                </div>
              </div>
            );
          ...
          {isAdmin && !testStarted && (
            <div className="mt-12 space-y-6">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-2xl font-bold">Pannello Admin - Test Head Judge</h2>
              </div>
      ...
      <Dialog open={editingIntro} onOpenChange={setEditingIntro}>
        <DialogContent>
          <DialogHeader><DialogTitle>Modifica Introduzione Test Head Judge</DialogTitle></DialogHeader>
      ...
      <Dialog open={editingSettings} onOpenChange={setEditingSettings}>
        <DialogContent>
          <DialogHeader><DialogTitle>Impostazioni Test Head Judge</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div>
